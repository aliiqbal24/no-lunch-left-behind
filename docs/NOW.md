...
- Baseline local game checkpoint: `5069fb5` — title and finale updates saved from previously uncommitted work.
...
| --- | --- | --- | --- | --- |
| Integration, reviews, final playthrough, release | Integration session | `main` / this checkout | `game/src/main.js`, `game/index.html`, `game/style.css`, this board | Active; no release approved |
...
After the owner picks the next scene or polish tasks, assign each session disjoint files and acceptance criteria here, then have each session create its own worktree/branch. Review each commit SHA and connect its hooks in `main`. The jam fork/entry JSON and exact-commit gate remain release tasks, not contributor tasks.
